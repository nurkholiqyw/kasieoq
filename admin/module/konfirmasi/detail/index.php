    ...
    if (bayar >= total) {
        ...
        xhr.onload = function() {
            if (xhr.status == 200) {
                ...
                var url = "print5.php?nm_member=<?php echo $_SESSION['admin']['nm_member']; ?>
										&bayar=" + bayar + 
										"&kembali=" + (bayar - total) + 
										"&total=" + total+ 
										"&jumlah_barang=" + jumlah_barang.join(",") + // Tambahkan jumlah_barang ke URL
                         				 "&total1=" + total1.join(",")+ // Tambahkan total1 ke URL
										"&id=" + id_pesanan.join(",") + 
